...
def test_disease_gene_example_conversion_to_hetmat(tmpdir):
    ...
    hetnet_adj = hetmatpy.matrix.metaedge_to_adjacency_matrix(graph, 'GaD', dense_threshold=0)
    hetmat_adj = hetmatpy.matrix.metaedge_to_adjacency_matrix(hetmat, 'GaD', dense_threshold=0)
    ...
    hetnet_adj = hetmatpy.matrix.metaedge_to_adjacency_matrix(graph, 'DaG', dense_threshold=0)
    hetmat_adj = hetmatpy.matrix.metaedge_to_adjacency_matrix(hetmat, 'DaG', dense_threshold=0)
    assert hetnet_adj[0] == hetmat_adj[0]  # row identifiers
    assert hetnet_adj[1] == hetmat_adj[1]  # column identifiers
    assert numpy.array_equal(hetnet_adj[2], hetmat_adj[2])  # adj matrices
