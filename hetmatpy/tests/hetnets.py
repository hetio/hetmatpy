...
hetnet_urls = {
    ...
    'disease-gene-example':
        format_github_url(
            repo_slug='dhimmel/hetio',
...
def get_graph(name, hetmat=False, directory=None):
    ...
    if name not in hetnet_io_cache:
        ...
        read_file = hetio.readwrite.open_read_file(url, text_mode=True)
        hetnet_io_cache[name] = read_file.read()
    writable = json.loads(hetnet_io_cache[name])
    graph = hetio.readwrite.graph_from_writable(writable)
    if not hetmat:
        return graph
